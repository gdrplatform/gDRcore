...
create_control_df <- function(df_,
  ...
  if (nrow(df_) != 0L) {
    ...
    df_ <- df_[, c("CorrectedReadout", col_intersect), with = FALSE]
    ...
    if (ncol(df_) > 1) {
      ...
      df_ <- stats::aggregate(df_[, out_col_name, with = FALSE],
              by = as.list(df_[, selected_columns, with = FALSE]),
              function(x) control_mean_fxn(x))
    } else if (ncol(df_) == 1) {
      # only ReadoutValue column exists (i.e. no 'Barcode')
      df_ <- data.table::data.table(control_mean_fxn(unlist(df_[, out_col_name, with = FALSE])))
