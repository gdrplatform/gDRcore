...
- fix time-course fits being bounded as relative viability, which
  clipped negative normalized growth-rate asymptotes to zero
- add per-normalization-type priors and asymptote bounds, replacing the
  GR/else branch
...
- switch from `lapply` into
  [`gDRutils::loop`](https://gdrplatform.github.io/gDRstyle/reference/loop.html)
  in create_SE
...
- switch to
  [`gDRutils::remove_drug_batch`](https://gdrplatform.github.io/gDRstyle/reference/remove_drug_batch.html)
  function
...
- allow using custom functions for calculating HSA and Bliss scores for
  combination data
...
- improve logic for normalization and identification of single-agent and
  matrix data with Drug3
...
- update annotation column names for cell line annotation as per changes
  in the gDRutils
...
- utilize
  [`gDRutils::apply_bumpy_function`](https://gdrplatform.github.io/gDRstyle/reference/apply_bumpy_function.html)
  in fit_SE
...
- switch from `gDRinternal` to `gDRinternalData` for internal
  annotations
...
- support an import of single agent data from GDS
  ...
  - minor refactor to use the ‘masked’ column. If it is not part of the
    initial dataframe it is added automatically - and set to FALSE
