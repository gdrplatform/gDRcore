...
normalize_data <-
  function(df_raw_data,
    ...
    df_end_mean <- aggregate(df_end_untrt[, "CorrectedReadout"],
                             by = as.list(df_end_untrt[, Keys$Endpoint]), function(x)
                               mean(x, trim = .25))
    ...
    df_day0_mean <- aggregate(df_day0[, "CorrectedReadout"],
                              by = as.list(df_day0[, Keys$Day0]), function(x)
                                mean(x, trim = .25))
    colnames(df_day0_mean)[dim(df_day0_mean)[2]] <- "Day0Readout"
    
    df_controls <-
      merge(df_end_mean, df_day0_mean[, setdiff(colnames(df_day0_mean),
                                                c(get_identifier("duration"), "Barcode"))], all.x = TRUE)
    if (length(setdiff(Keys$Endpoint, Keys$Day0)) > 0) {
      WarnMsg <-
        paste(
          "Not all control conditions found on the day 0 plate,",
...
identify_keys <- function(df) {
  keys <- list(
    Trt = setdiff(colnames(df), "Barcode"),
    DoseResp = setdiff(colnames(df),  "Barcode"),
    Endpoint = colnames(df)[c(
      -agrep("Concentration", colnames(df)),-agrep(get_identifier("drug"), colnames(df)),-agrep(get_identifier("drugname"), colnames(df))
    )]
  )
  ...
  for (k in keys[["Endpoint"]]) {
    ...
    if (all(is.na(df[df[, get_identifier("duration")] == 0, k]))) {
...
cleanup_metadata <- function(df_metadata, log_str) {
  ...
  gCLs <-
    gneDB::annotateCLIDs(unique(df_metadata[, get_identifier("cellline")]))[,
                                                                            c(get_identifier("DB_cell"),
                                                                              "celllinename",
                                                                              "primarytissue",
                                                                              "doublingtime")]
  
  colnames(gCLs) <-
    c(get_identifier("cellline"), get_header("add_clid"))
  ...
  df_metadata <-
    merge(
      df_metadata,
      gCLs,
      by.x = get_identifier("cellline"),
      by.y = get_identifier("DB_cell"),
      all.x = TRUE
    )
  ...
  for (i in agrep(get_identifier("drug"), colnames(df_metadata))) {
    # correct case issues
  ...
  gDrugs <-
    gCellGenomics::getDrugs()[, c(get_identifier("DB_drug"), "gcsi_drug_name")]
  
  # -----------------------
  gDrugs[, 1] <-
    substr(gDrugs[, 1], 1, 9) # remove batch number from get_identifier("DB_drug")
  ...
  Gnbrs <-
    unique(unlist(df_metadata[, agrep(get_identifier("drug"), colnames(df_metadata))]))
  ...
  if (any(bad_Gn)) {
    ...
    if (any(ok_Gn)) {
      WarnMsg <-
        paste(
          "Drug ",
          paste(Gnbrs[ok_Gn & bad_Gn], collapse = " ; "),
          " not found in gCSI database; use G# as DrugName"
        )
      gDrugs <-
        rbind(gDrugs, data.frame(drug = Gnbrs[ok_Gn & bad_Gn],
  ...
  for (i in grep(paste0(get_identifier("drug"), "_\\d"), colnames(df_metadata))) {
    df_metadata[is.na(df_metadata[, i]), i] <-
      get_identifier("untreated_tag")[1] # set missing values to Untreated
    gDrugs_ <- gDrugs
    colnames(gDrugs_)[2] <-
      paste0(colnames(gDrugs_)[2], substr(colnames(df_metadata)[i], 8, 12))
  ...
  for (i in agrep("Concentration", colnames(df_metadata))) {
    trt_n <- ifelse(regexpr("_\\d", colnames(df_metadata)[i]) > 0,
                    substr(colnames(df_metadata)[i], 15, 20),
                    1)
    DrugID_col <-
      ifelse(trt_n == 1,
             get_identifier("drug"),
             paste0(get_identifier("drug"), "_", trt_n))
    df_metadata[df_metadata[, DrugID_col] %in% get_identifier("untreated_tag"), i] <-
      0 # set all untreated to 0
    
    DrugID_0 <-
      setdiff(unique(df_metadata[df_metadata[, i] == 0, DrugID_col]), get_identifier("untreated_tag"))
    DrugID_0 <- DrugID_0[!is.na(DrugID_0)]
    if (length(DrugID_0) > 0) {
      WarnMsg <- paste("Some concentration for ",
                       DrugID_col,
                       " are 0: ",
