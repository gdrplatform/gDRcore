...
df_normalized = data.frame( CellLineName = as.vector(t(matrix(c('COV318', 'HCC2218'), 2, 270))),
                    Tissue = as.vector(t(matrix(c('Ovary', 'Breast'), 2,270))),
                    Duration = 72,
                    clid = as.vector(t(matrix(c('CL586056', 'CL131828'), 2,270))),
                    ...
                    ReadoutValue = 100,
                    DivisionTime = as.vector(t(matrix(c(57,85), 2, 270))),
                    ReferenceDivisionTime = as.vector(t(matrix(c(57,85), 2, 270))),
                    BackgroundValue = 0)
...
df_normalized$CorrectedReadout = 100 *
...
df_normalized$CorrectedReadout[idx] = df_normalized$CorrectedReadout[idx] -
                            .15 * (100 - df_normalized$CorrectedReadout[idx])

df_normalized$CorrectedReadout = pmin(df_normalized$CorrectedReadout, 103)

# add some noise
# set.seed(1)
# df_normalized$CorrectedReadout = df_normalized$CorrectedReadout + runif(dim(df_normalized)[1],-4,3)
df_normalized$ReadoutValue = df_normalized$CorrectedReadout + df_normalized$BackgroundValue

df_normalized$RelativeViability = round(df_normalized$CorrectedReadout / 100, 4)
...
df_normalized$WellRow = sort(rep(LETTERS[1:8],12))[-6:-1]
df_normalized$WellColumn = rep(1:12,8)[-6:-1]
levels(df_normalized$Barcode) = paste0('Plate',1:6)

df_manifest = unique(df_normalized[, c('Barcode', 'Template', 'Duration', 'clid')])
df_data = df_normalized[,c('Barcode', 'WellRow', 'WellColumn', 'ReadoutValue', 'BackgroundValue')]
df_treatment = unique(df_normalized[, c('Template', 'WellRow', 'WellColumn', 'Gnumber',
