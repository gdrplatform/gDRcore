---
title: "Custom fitting with apply_custom_fit()"
author: "gDR team"
output: BiocStyle::html_document
vignette: >
  %\VignetteIndexEntry{Custom fitting with apply_custom_fit()}
...
`apply_custom_fit()` and `apply_custom_fits()` let you plug **any R function**
...
- `apply_custom_fits()` applies N fit functions in a **single** BumpyMatrix
...
sa_two <- sa_out |>
  apply_custom_fit(extra_fn, "single-agent",
                   output_assay = "custom_summary",
                   fit_source = "extremes")
...
hill_with_summary <- apply_custom_fit(
  sa_se,
  fit_fn = fit_drug_response_metrics,
  data_type = "single-agent",
  output_assay = "custom_hill",
...
# Efficient multi-fit: apply_custom_fits()

When several fit functions operate on the **same input assay**, use
`apply_custom_fits()` to traverse each BumpyMatrix cell **once** and apply
all functions in that single pass.

```{r multi-fit}
combo_multi <- apply_custom_fits(
  combo_se,
  fit_fns = list(
...
`apply_custom_fit()` calls but without the overhead of a second unsplit +
iteration.
