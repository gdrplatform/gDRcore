...
These functions, the fit profile registry and `fit_drug_response_metrics()` now
live in **gDRutils**, next to the curve-fitting maths they are built on.  gDRcore
re-exports them for one release cycle, so the code below keeps working as written;
call them as `gDRutils::apply_fit()` in new code.
...
|---|---|---|
| 1+2 | `apply_combo_sa_fits()` | `Metrics` |
| 3 | `apply_combo_excess()` | `excess` |
| 4 | `apply_combo_isobolograms()` | `isobolograms`, `all_iso_points` |
| 5 | `apply_combo_scores()` | `scores` |
...
Live-cell imaging experiments (e.g. Incucyte) produce per-well cell counts at
multiple time points rather than endpoint readouts.  The standard `fit_SE()`
pipeline is not appropriate for this layout.  `fit_SE.timecourse()` provides a
dedicated two-stage pipeline:
...
`fit_SE.timecourse()` expects a `SummarizedExperiment` with a `LogFoldChange`
assay — the output of `normalize_SE(se, data_type = "time-course")`:

```{r timecourse-setup, eval = FALSE}
...
```{r timecourse-output, eval = FALSE}
...
combination, with the same fit columns as `fit_SE()`: `ec50`, `xc50`, `h`,
`r2`, `x_mean`, `x_AOC`, `N_conc`, `fit_type`, etc.

## Custom fitting for time-course data

Both stages are pluggable, so the custom fit interface covers time-course data
the same way it covers single-agent and combination screens.

Stage 2 takes a `fit_fn` with the usual contract — it receives one
`normalization_type = "NGR"` slice of the `GrowthRates` assay, with
`NormalizedGrowthRate` as the response and `Concentration` as the x axis:

```{r timecourse-custom-fit, eval = FALSE}
auc_fn <- function(dt) {
...
Stage 1 takes a `rate_fn`, which receives the timepoints of a single well
within a single window and returns one growth rate in doublings per day.
Replace it when the default `lm(LogFoldChange ~ Duration)` slope is not the
right estimator — for example for screens with only two usable timepoints, or
when the growth phase has to be selected per well:

```{r timecourse-custom-rate, eval = FALSE}
...
The stages can also be run separately, which is the same two-step workflow as
`average_SE()` → `apply_fit()` for single-agent data.  `compute_growth_rates()`
returns the growth rate table — one row per `(CellLine, Drug, Concentration,
partner slots, period)`, **controls included**, with `GrowthRate`,
`sd_GrowthRate` and `rate_0` — and `growth_rates_to_se()` turns it into the
stage 2 input:

```{r timecourse-two-step, eval = FALSE}
...
Note that `fit_SE.timecourse()` keeps the `GrowthRates` assay in its output, so
an already-fitted object can be re-fitted with a different `fit_fn` without
recomputing stage 1.

## Key design notes

- **`normalization_type = "NGR"`** (NormalizedGrowthRate) — distinct from
  `"GR"` (GR value) and `"RV"` (Relative Viability) used in single-agent
  screens.  Time-course data has no RV/GR split.
- **Half-open windows `[start, end)`** — contiguous periods partition the
  time-course: the shared boundary belongs to the later window and `t = 0` is
  kept.
- **All partner-drug slots in the row key** — each combination arm at each
  partner concentration gets its own dose-response curve.
- **`untreated_tag` from `gDRutils`** — DMSO detection uses
  `get_env_identifiers("untreated_tag")` rather than hardcoded strings.
- **Two profiles, one `apply_fit()` pass** — `"time-course"` supplies the
  default input assay of stage 1, `"time-course-metrics"` drives the stage 2
  slicing.  Stage 1 aggregates replicates and normalises against control wells
  that live in *other* BumpyMatrix cells, so it cannot be expressed as an
  `apply_fit()` pass; that is why it is pluggable through `rate_fn` instead.
  See GDR-3506 for the planned multi-stage profile model.
