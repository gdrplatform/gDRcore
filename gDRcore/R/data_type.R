...
identify_data_type <- function(df,
  ...
  for (idp in seq_len(nrow(drug_pairs))) {
    ...
    treated <- vapply(parallel(df_matching[, drugs_ids, drop = FALSE],
...
split_raw_data <- function(df,
  ...
  if (length(cotrt_types) > 0) {
    df_list[cotrt_types] <- parallel(cotrt_types, function(x) {
  ...
  if ("single-agent" %in% names(df_list)) {
    sa_idx <- parallel(grep(drug_ids[["concentration"]], drug_ids, value = TRUE), function(x)
