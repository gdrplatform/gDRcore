...
map_df <- function(trt_md, 
  ...
  out <- parallel(seq_along(trt_rnames), function(i) {
    treatment <- trt_rnames[i]
    refs <- parallel(present_ref_cols, function(y) {
...
.map_references <- function(mat_elem) {
  ...
  has_tag <- as.data.frame(parallel(drug_cols, function(x) grepl(pattern, x)))
  ...
  if (any(is_ref)) {
    ...
    trt <- parallel(valid, function(x) trt[, c(clid, x)])
    trt <- do.call(paste, do.call(rbind, parallel(trt, function(x) setNames(x, names(trt[[1]])))))
    
    ref <- parallel(valid, function(x) ref[, c(clid, x)])
    ref <- do.call(paste, do.call(rbind, parallel(ref, function(x) setNames(x, names(ref[[1]])))))
