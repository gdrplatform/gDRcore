...
fit_SE.combinations <- function(se,
  ...
  for (i in seq_len(ncol(se))) { # each cell line
    for (j in seq_len(nrow(se))) { # each drug pair
      ...
      # TODO: are the controls the single agents? 
      measured_ctrl <- gDRutils::convert_se_ref_assay_to_dt(se[combo$rows, i])
      measured <- as.data.frame(rbind(avg_combo, measured_ctrl))
      
      print(sprintf("Calculation for cell line %s treated with %s x %s", 
                    SummarizedExperiment::colData(se)$CellLineName[i], 
                    combo$condition["DrugName"], combo$condition["DrugName_2"]))

      # Omitting for simple case of data being presented properly as drug 1 and drug2.
      #measured <- swap_indices(se) 

      ###############
      # Impute
      ###############
      ## I think this is probably trying to figure out what concentration pairs don't have readouts
      ## and then try to fill w nearby values?
        ...
        if (nrow(idx_na) > 0) {
          for (i in seq_len(nrow(idx_na))) {
            ...
            imputed_matrix[idx_na[i, "row"], idx_na[i, "col"]] <- mean(close_mx, na.rm = TRUE) # use a smoothing strategy
          }
        }
      ...
      all_conc1 <- unique(avg_combo[, id])
      all_conc2 <- unique(avg_combo[, id2])

      # Get the single agent fits
      ...
      n <- 1
      for (conc1 in setdiff(all_conc1, 0)) {
      ...
      for (conc2 in setdiff(all_conc2, 0)) {
      ...
      # Fit diagonals
      ###############
      #fit_codilutions(mx_response) # TODO: Fill me back in.

      ###########################
      # Calculate the mean (measured, imputed, and row/column/diagonal fitted data)
      ###########################
      X <- list(measured, imputed, cotrt_matrix)
      Y <- do.call(cbind, X) # TODO: Merge by concentration pairs.
      Y <- array(Y, dim=c(dim(X[[1]]), length(X)))
      mean_matrix <- apply(Y, c(1, 2), mean, na.rm = TRUE)

      ################################
      # Calculate combination scores
      ################################
      hsa <- calculate_HSA(mean_matrix)
      hsa_excess <- hsa - mean_matrix

      bliss <- calculate_Bliss(mean_matrix)
      bliss_excess <- bliss - mean_matrix

      # decide on the isobologram cutoff levels for the Loewe model 
      loewe <- calculate_Loewe(mean_matrix)
...
#' function to fit the data by rows or columns and replace measured values by fitted values
#' @keywords internal
fit_matrix <- function(mx, idx, normalization_type) {
  single_agent <- mx[idx, 1]
  concs <- as.numeric(concentrations)[-1]
  try({
    fit_res <- fit_cotreatment(concs, single_agent, reverse = FALSE) 
  }, error = function(e) {
    # if failed and values are increasing, fit a reverse sigmoidal
    if (is.na(fit_res$x_0) && stats::median(diff(mx[idx, -1])) > 0 && mean(diff(mx[idx, -1])) > 0) {
      fit_res <- fit_cotreatment(concs, single_agent, reverse = TRUE)
    }
  })
  mx[idx, -1] <- fit_res
  list(mx = mx, fit_res = fit_res)
}


#' @keywords internal
fit_cotreatment <- function(concs, single_agent, normalization_method, reverse = FALSE) {
  e_0 <- GR_0 <- single_agent
  df_ <- data.frame(concs, single_agent)
  colnames(df_) <- c("Concentration", normalization_type) # Note that fit_curves is expecting a "RelativeViability" and "GRvalue" column instead of a "RV" and "GR" column
  if (reverse) {
    df_[, normalization_type] <- 1 - df_[, normalization_type]
    e_0 <- GR_0 <- 1 - single_agent
  }
  ...
  out <- gDRutils::logistic_4parameters(concs,
    fit_res$x_inf, fit_res$x_0, fit_res$c50, fit_res$h)
  out
}
...
calculate_HSA <- function(mean_matrix) {
  # Currently this treats it as a matrix to do the mins.
  single_agent1 <- mean_matrix # single agent 1
  single_agent2 <- mean_matrix # single agent 2
  hsa_mx <- all_mx$mx_mean
  hsa_mx[-1, -1] <- pmin(t(matrix(t(hsa_mx[1, -1]), ncol(hsa_mx) - 1, nrow(hsa_mx) - 1)),
                      matrix(hsa_mx[-1, 1], nrow(hsa_mx) - 1, ncol(hsa_mx) - 1))
  hsa_mx
...
swap_indices <- function(se) {
# Secondary drug for some combinations becomes primary drug when viewed as single-agent (conc1=0), so swap. 
# swap the data into the Gnumber and Gnumber_2 such that it can form a matrix
swap_idx <- flat_data[, gDRutils::get_SE_identifier(se, "drugname")] == 
  combo$condition[[paste0(gDRutils::get_SE_identifier(se, "drugname"), "_2")]] & 
  flat_data$Gnumber_2 %in% gDRutils::get_SE_identifier(se, "untreated_tag")
temp_df <- flat_data[swap_idx, ]
cols <- c("Gnumber", "DrugName", "drug_moa", "Concentration",
          "Gnumber_2", "DrugName_2", "drug_moa_2", "Concentration_2")
new_cols <- c(cols[5:8], cols[1:4])
temp_df[, cols] <- temp_df[, new_cols] 
# Append the bottom left entry for full matrix creation. 
temp_df2 <- .create_combo_control()
flat_data <- rbind(temp_df2, flat_data[!swap_idx, ], temp_df)
flat_data
}


fit_codilutions <- function(mx_response) {
# replace with fits by diagonals (co-dilutions with different ratios)
...
if (length(conc_ratio) != 1 || n_conc_codil < 4) { # check that ratios are respected
  ...
  for (i in -(nrow(mx_response) - 1):(ncol(mx_response) - 1)) {
    ...
    if (nrow(idx) < 4) {
      ...
      # add fitted data 
      out <- rbind(all_fits$by_codil, cbind(data.frame(
        conc_ratio = as.numeric(rownames(mx_fit[["by_codil"]])[idx$row_idx[1]]) / 
              as.numeric(colnames(mx_fit[["by_codil"]])[idx$col_idx[1]])),
            fit_res))
    }
