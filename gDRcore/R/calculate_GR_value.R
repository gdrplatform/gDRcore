...
#' @param ndigit_rounding integer specifying the number of digits to use for 
#' calculation rounding.
#' @param duration numeric value specifying the length of time the cells were 
#' treated (in hours).
#' @param ref_div_time numeric value specifying the reference division time for 
#' the cell line in the experiment.
#' @param cap numeric value representing the value to cap the highest allowed 
#' relative viability at.
#'
#' @return numeric vector containing GR values, one value for each element of 
#' the input vectors.
#' 
#' @details Note that this function expects that all vectorized numeric vectors 
#' should be of the same length. \code{calculate_GR_value} will try to greedily 
#' calculate a GR value. If no day 0 readouts are available, the \code{duration}
#'  and \code{ref_div_time} will be used to try to back-calculate a day 0 value 
#'  in order to produce a GR value.
#'
#' In the case of calculating the reference GR value from multiple reference 
#' readout values, the vectorized calculation is performed and then the 
#' resulting vector should be averaged outside of this function. 
#'
...
calculate_GR_value <- function(rel_viability, 
  ...
  if (length(unique(vapply(args_to_validate, FUN = length, numeric(1)))) 
      != 1L) {
    stop(
      "unequal vector lengths: rel_viability, 
  ...
  if (any(is.na(day0_readout))) {
    ## Back-calculate the day0_readout using the reference doubling time and 
...
calculate_time_dep_GR_value <- function(corrected_readout,
                                        ...
                                        ndigit_rounding) {
  round(
    2 ^ (log2(corrected_readout / day0_readout) /
         log2(untrt_readout / day0_readout)), 
    ndigit_rounding
  ) - 1
}
...
calculate_endpt_GR_value <- function(rel_viability, 
                                     ...
                                     ndigit_rounding) {
  round(
    2 ^ (1 + (log2(pmin(cap, rel_viability)) / (duration / ref_div_time))), 
    ndigit_rounding
  ) - 1
}
