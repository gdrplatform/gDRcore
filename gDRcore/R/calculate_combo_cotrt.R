...
df_all_combo = do.call(rbind, 
        lapply(metadata(SE)$drug_combinations, function(x) as.data.frame(x$condition)))
rownames(df_all_combo) = paste0(df_all_combo$Gnumber, '_', df_all_combo$Gnumber_2, '_', df_all_combo$Concentration_2)
...
for (idc in seq_len(length(metadata(SE)$drug_combinations))) {
    ...
    for (iCL in seq_len(ncol(SE))) {
        ...
        flat_data <- gDRutils::convert_se_assay_to_dt(SE[combo$rows, iCL], "Metrics", metric_type = 'both')
