...
the assays of the SEs comprises the series themselves as a table (data.frame in practice). Although not all elements need to have
...
```{r}
data_path <- system.file("extdata/data1", package = "gDRimport")
data_list <- list.files(data_path)
manifest_data <- list.files(data_path, "^manifest", full.names = TRUE)
template_data <- list.files(data_path, "^Template", full.names = TRUE)
raw_data <- list.files(data_path, "^RawData", full.names = TRUE)

basename(c(manifest_data, template_data, raw_data))
```

Provided dataset needs to be merged into the one `data.frame` object to be able to run gDR pipeline. This process can be done using two functions --
`gDRimport::load_data` and `gDRcore::merge_data()`.

```{r, nclude=FALSE}
loaded_data <- gDRimport::load_data(manifest_data, template_data, raw_data)
input_df <- gDRcore::merge_data(loaded_data$manifest, loaded_data$treatments, loaded_data$data)
head(input_df)
```
